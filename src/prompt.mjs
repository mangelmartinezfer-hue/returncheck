...
For a determinate verdict fill policy and evidence. evidence.exact_clause MUST be a verbatim quote copied from the policy text (its own language, never paraphrased). CRITICAL — quote the OPERATIVE clause, the single sentence that directly establishes the verdict: for a positive verdict, the sentence stating the return/exchange window (and, if a number of days applies, that sentence MUST contain that number); for NO, the sentence stating the item cannot be returned / is final sale / is excluded. Do NOT quote a secondary condition (e.g. "must be unused / in original packaging"), a fee sentence (e.g. restocking), or a shipping line as the exact_clause — those do not prove returnability and will be rejected. If two windows appear, quote the one that applies to THIS request. Map item_condition to the enum (opened/used -> UsedCondition when used is accepted; unopened -> NewCondition); never copy the raw request string. In NotPermitted with no refund, refund_type may be null and return_fees null. Include restocking_fee.currency only when type="amount". merchant_return_days: integer or null. Do not compute deadline_date (the caller does it). exceptions[] are short snake_case tags.`;
